...
async function startRealtimeInfer(videoEl) {
    ...
    inferInterval = setInterval(async () => {
        ...
        try {
            ...
            const resp = await fetch(`${API_BASE_URL}/infer-frame`, { 
                method: 'POST', 
                body: form,
...
async function sendConfirmedWords(words = confirmedWords) {
    ...
    const response = await fetch(`${API_BASE_URL}/process-confirmed-words`, {
        method: 'POST',
...
async function composeReverseVideo() {
    ...
    try {
        ...
        const resp = await fetch(`${API_BASE_URL}/reverse-translate-video`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function prefetchSegmentClip(seg) {
    ...
    try {
        const resp = await fetch(`${API_BASE_URL}/reverse-translate-segment`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function startReverseSyncForYouTubeQueueMode(player, reverseVideoEl, transcriptSegments) {
    ...
    ;(async () => {
        for (let i = 0; i < ytReverse.segments.length; i++) {
            ...
            try {
                const resp = await fetch(`${API_BASE_URL}/tokenize-text`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
...
async function startRealtimeLetterInfer(videoEl) {
    ...
    letterInferInterval = setInterval(async () => {
        ...
        try {
            ...
            const resp = await fetch(`${API_BASE_URL}/infer-letter`, { 
                method: 'POST', 
                body: form,
...
async function startAlphabetInference(videoEl) {
    ...
    alphabetInferInterval = setInterval(async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/infer-letter`, {
                method: 'POST',
                body: form,
